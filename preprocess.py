...
def main(args):
    data_pth = "data/%s/%s_on_sentiment" % (args.data_name,args.data_name)
    res_pth = "results/%s/%s_on_sentiment" % (args.data_name,args.data_name)
    for split in ["train", "dev", "test"]:
        #### For sentiment data
        pth0 = "sentiment.%s.0" % split
        pth1 = "sentiment.%s.1" % split
        outpth = "%s_sentiment_data.txt" % split
        _outpth = "_%s_sentiment_data.txt" % split
    ...
    data_pth = "data/%s/%s_on_tense" % (args.data_name, args.data_name)
    res_pth = "results/%s/%s_on_tense" % (args.data_name, args.data_name)
    for split in ["train", "dev", "test"]:
        #### For tense data
        pth0 = "tense.%s.0" % split
        pth1 = "tense.%s.1" % split
        outpth = "%s_tense_data.txt" % split
        _outpth = "_%s_tense_data.txt" % split
        pth0 = os.path.join(data_pth, pth0)
        pth1 = os.path.join(data_pth, pth1)
        outpth = os.path.join(data_pth, outpth)
        _outpth = os.path.join(data_pth, _outpth)
